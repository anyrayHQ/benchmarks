...
|---|---|---|--:|--:|--:|
| Access log (500 requests) — "find the failing requests" | `relevance_filter` | `keepChars=3000, roles=user` | 26,153 | 1,074 | **96%** |
| SRE incident — "why did checkout p99 spike at 10:05?" | `relevance_filter` | `keepChars=12000, roles=user` | 26,385 | 4,309 | **84%** |
| Synonym-gap logs — "find the resource-exhaustion event" (OOM/cgroup) | `relevance_filter` | `keepChars=3000, roles=user, semanticRerank=true, semanticWeight=0.7` | 2,963 | 1,180 | **60%** |
| JSON array (500 items) — "which orders failed and why?" | `context_compression` | `roles=user` | 74,460 | 16,012 | **78%** |
| Orders dump (tool result) — "which orders failed and why?" | `context_compression` | `(defaults)` | 17,989 | 10,485 | **42%** |
| Metrics series (tool result) — "find the latency spike" | `context_compression` | `(defaults)` | 12,141 | 7,097 | **42%** |
