...
// `npm run summary` after `npm run bench:all` / `npm run quality:all`.
...
export function renderSummary(cfg = loadConfig()) {
  let totBefore = 0;
  let totAfter = 0;
  let specialCount = 0; // guardrail/special-accounting workloads (cache/param/diagnostic) — not char reduction
  const perSuite = [];
  const allSaved = [];
  const quality = [];
  ...
  const lines = [
    ...
    `Across ${perSuite.reduce((a, s) => a + s.n, 0)} synthetic agent/coding workloads, the Anyray ` +
      ...
      `${median(allSaved)}% per workload) while preserving the ` +
    ...
    ...(specialCount
      ? [
          '',
          `_(${specialCount} guardrail workloads — semantic cache, runaway-output ` +
            'caps — use special accounting rather than whole-request token reduction and are ' +
            'reported separately.)_',
        ]
      : []),
    '',
    '_Synthetic data only (privacy-preserving). Numbers are reproducible: `npm run bench:all` ' +
      'then `npm run quality:all`, then `npm run summary`. See `VALIDATION.md` for methodology._',
  ];
