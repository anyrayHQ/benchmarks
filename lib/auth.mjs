...
/** Same identity requirement for the native /v1/messages route: top-level `system`. */
export function withClaudeIdentitySystem(system, auth) {
  if (!(auth?.mode === 'passthrough' && auth.provider === 'anthropic' && auth.upstreamToken)) {
    return system;
  }
  const text = typeof system === 'string' ? system : '';
  if (text.startsWith('You are Claude Code')) return system;
  return text ? `${CLAUDE_CODE_SYSTEM}\n\n${text}` : CLAUDE_CODE_SYSTEM;
}
