...
export class OptimizerClient {
   ...
   */
  /**
   * Like `optimize`, but carries the session's decision pins.
   *
   * The gateway sends `pins` on every turn and persists whatever comes back; the
   * optimizer itself stays stateless. The field is not cosmetic — the cache-safety
   * guard uses it to locate the cache boundary, and WITHOUT it the guard cannot
   * tell a settled edit from a fresh one and stands every eliding strategy down
   * (`suppressedKinds: [{reason: "cache_guard"}]`). A caller that omits pins
   * therefore measures a no-op and would report it as "the optimizer does not
   * help here", which is a statement about the caller, not the optimizer.
   */
  async optimizeWithPins(request, pins, { endpoint, metadata } = {}) {
